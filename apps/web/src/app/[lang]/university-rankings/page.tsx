...
import { Globe, ExternalLink, Search, ArrowRight, BookOpen } from 'lucide-react';
...
const medalColors: Record<number, { bg: string; border: string; text: string; label: string }> = {
  1: { bg: 'bg-amber-50', border: 'border-amber-300', text: 'text-amber-600', label: '🥇' },
  2: { bg: 'bg-slate-50', border: 'border-slate-300', text: 'text-slate-500', label: '🥈' },
...
export default function GlobalUniversityRankingsPage() {
  ...
  const showTop3 = search === '' || filteredTop3.length > 0;
    ...
    <MainLayout>
      ...
      <Container className="py-10 sm:py-14">
              ...
              {[top3[1], top3[0], top3[2]].map((u) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center text-sm font-bold text-white mb-3 ${
                      u.rank === 1 ? 'bg-amber-500' : u.rank === 2 ? 'bg-slate-400' : 'bg-orange-400'
                    }`}>
                      {getInitials(u.name)}
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 mb-4">
                      <div className="h-1.5 w-20 bg-gray-200 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${u.rank === 1 ? 'bg-amber-500' : u.rank === 2 ? 'bg-slate-400' : 'bg-orange-400'}`} style={{ width: `${u.score}%` }} />
                      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden mb-12">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="font-semibold text-gray-800 text-sm">
              {search ? `Results for "${search}"` : 'Ranks 4–25'}
            </h2>
          </div>
        ...
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Rankings by Country</h2>
            <p className="text-sm text-gray-500 mt-0.5">Top universities in each country, ranked individually.</p>
          </div>
        </div>
